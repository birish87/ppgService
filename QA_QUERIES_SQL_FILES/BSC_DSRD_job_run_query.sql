...
VALUES ('BSC_DSRD_job_run_query', $$-- 'd' = e/d
-- 'bsc' = bsc
BEGIN;
...
left join rpt_bsc_dsr_claim_gpi_outcome_prep_data rhdcgop on aaam.id = rhdcgop."ExistingMedInternalID" and rhdcgop.fill_type = 'New' and rhdcgop."ProductLine" = 'd'
where aaa.meets_criteria = true --only DSR with saving
	...
	and ((rhdcgop."ExistingMedInternalID" is null and lower(aaa.dsr_type) = 'd' and rdtd.transaction_id is not null and cast(rdtd.published_date::date +'180 day'::interval as date) >= cast(date_trunc('month', current_date - '1 month'::interval) as date))
		or (rhdcgop."ExistingMedInternalID" is null and lower(aaa.dsr_type) = 'e' and cast(aaa.inserted_at::date +'180 day'::interval as date) >= cast(date_trunc('month', current_date - '1 month'::interval) as date))
...
left join rpt_bsc_dsr_claim_gpi_outcome_prep_data rhdcgop on aaam.id = rhdcgop."ExistingMedInternalID" and rhdcgop.fill_type = 'New' and rhdcgop."ProductLine" = upper('DSR-'||'d') and stop_tracking = false
where aaa.meets_criteria = true --only DSR with saving
	and lower(pay."name") = 'bsc'
	and lower(aaa.dsr_type) = 'd'
	and aaa.ehr_organization_id not in ('1','39') 
...
where 
	(('d' = 'e' and thlmc.date_of_service::date >= alt.report_created_date::date) or ('d' = 'd' and thlmc.date_of_service::date > alt.published_date::date)) --DSR is not possible in user hand on DSR create date because clinical review
...
select distinct thlmc.new_patient_identifier as patient_identifier
	...
	,case 
		...
		when 'd' = 'e' then alt.report_created_date::date-thc.date_of_service::date 
	end as lookback_days
	,thlmc.med_super_concept as mml_med_super_concept
	,alt.is_first_fill
from temp_altermedlist alt
join temp_bsc_claims thlmc on alt.patient_identifier = thlmc.new_patient_identifier 
	and (('d' = 'e' and thlmc.date_of_service::date >= alt.report_created_date::date) or ('d' = 'd' and thlmc.date_of_service::date > alt.published_date::date))
...
left join temp_bsc_claims thc on alt.patient_identifier = thc.new_patient_identifier 
    and (('d' = 'e' and thc.date_of_service::date < alt.report_created_date::date) or ('d' = 'd' and thc.date_of_service::date <= alt.published_date::date))
...
select distinct ttp.patient_identifier 
	...
	,case 
		...
		when 'd' = 'e' and ttp.change_type = 'Tier 1' then ttp.report_created_date||'z'||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id
		when 'd' = 'e' then ttp.report_created_date||'a'||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id
...
where alt.is_first_fill = false --no first fill yet
		and (('d' = 'd' and cast(alt.published_date::date +'180 day'::interval as date) >= thlmc.date_of_service::date) 
			or ('d' = 'e' and cast(alt.report_created_date::date +'180 day'::interval as date) >= thlmc.date_of_service::date))
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
		)
...
select distinct ttp.patient_identifier 
	...
	,case 
		...
		when 'd' = 'e' then ttp.report_created_date||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id
	end as priority_index
...
where alt.is_first_fill = false
		and (('d' = 'd' and cast(alt.published_date::date +'180 day'::interval as date) >= thlmc.date_of_service::date) 
			or ('d' = 'e' and cast(alt.report_created_date::date +'180 day'::interval as date) >= thlmc.date_of_service::date))-- First Fill DOS must be less than or equal to the published date + 180 days 
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
		) 
...
join temp_emf_prep tep on ('d' = 'd' and ef.published_date::text||ef.med_id::text = tep.most_recent_ci) or ('d' = 'e' and ef.report_created_date::text||ef.med_id::text = tep.most_recent_ci)
left join temp_new_ci_list tncpl on ef.med_id = tncpl.med_id or ef.mml_record_id = tncpl.mml_record_id
left join temp_recurring_list tck on ef.med_id = tck.med_id or ef.mml_record_id = tck.mml_record_id
where tncpl.med_id is null and tck.med_id is null
UNION ALL 
...
insert into rpt_bsc_dsr_claim_gpi_outcome_prep_data
select t.*,false as stop_tracking
from temp_output t
...
--select * from rpt_bsc_dsr_claim_gpi_outcome_prep_data
