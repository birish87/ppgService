...
VALUES ('DSRD_Horizon_JOB_RUN_QUERY_NEW', $$-- :payer = horizon
BEGIN;
------------Begin Alter List----------------
...
select distinct 
lower('hzn'||trim(aaa.member_id)||aaa.dob::date) as patient_identifier --use XXX to avoid wrong matching
...
left join rpt_hzn_dsrd_claim_gpi_outcome_prep_data rhdcgop on aaam.id = rhdcgop."ExistingMedInternalID" and rhdcgop.fill_type = 'New'
where aaa.meets_criteria = true 
	and lower(pay."name") = 'horizon'--:payer 
	and lower(aaa.dsr_type) = 'd'
	and rdtd.transaction_id is not null 
	and aaa.ehr_organization_id not in ('1','39') and aaa.ehr_organization_id not ilike ('%test%') --exclude test data
	and ((rhdcgop."ExistingMedInternalID" is null and rdtd.transaction_id is not null and cast(rdtd.published_date::date +'180 day'::interval as date) >= cast(date_trunc('month', current_date - '1 month'::interval) as date))
		or (rhdcgop."ExistingMedInternalID" is not null and cast(rhdcgop."ClaimDate"::date +'365 day'::interval as date) >= cast(date_trunc('month', current_date - '1 month'::interval) as date)))
;
...
where rhch.date_of_service::date between cast((date_trunc('month', current_date)- '1 second'::interval)- '485 day'::interval as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)
--where rhch.date_of_service between cast((date_trunc('month', current_date)- '1 second'::interval)- '300 day'::interval as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)
;
...
where thlmc.date_of_service::date > alt.published_date::date --DSR is not possible in user hand on DSR create date because clinical review
...
drop table if exists temp_hzn_existing_med_found_keys;
create temp table temp_hzn_existing_med_found_keys as
select distinct med_id,mml_claim_date
from temp_hzn_existing_med_found_list
;

create index "_I_temp_hzn_existing_med_found_keys_med_id" on temp_hzn_existing_med_found_keys using btree(med_id,mml_claim_date)
;
...
select distinct thlmc.new_patient_identifier as patient_identifier
	...
	,alt.published_date::date-thc.date_of_service::date as lookback_days
	,thlmc.med_super_concept as mml_med_super_concept
	,alt.is_first_fill
from temp_altermedlist alt
join temp_hzn_claims thlmc on alt.patient_identifier = thlmc.new_patient_identifier 
	and thlmc.date_of_service::date > alt.published_date::date 
...
left join temp_hzn_claims thc on alt.patient_identifier = thc.new_patient_identifier 
    and thc.date_of_service::date <= alt.published_date::date
	and trim(lower(alt.med_display_name)) <> trim(lower(thc.med_name))
	and ((thlmc.gpi_12 is not null 
			and thc.gpi_12 is not null 
			and thlmc.gpi_12 = thc.gpi_12
			and upper(thlmc.med_msc) = upper(thc.med_msc))) --resulting medication was not filled in the past 120 days
left join temp_hzn_existing_med_found_keys themdk on alt.med_id = themdk.med_id and themdk.mml_claim_date::date >= thlmc.date_of_service::date 
where alt.is_first_fill
		...
		--and cast(alt.first_fill_claim_date +'180 day'::interval as date) >= thlmc.date_of_service::date-- Recurring Fill DOS must be less than or equal to the First Fill DOS + 180 days 
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			)
...
select distinct ttp.patient_identifier 
	...
	,ttp.is_first_fill
	,case 
		when ttp.change_type = 'Tier 1' then ttp.published_date||'z'||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id
		else ttp.published_date||'a'||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id
...
where alt.is_first_fill = false --no first fill yet
		and cast(alt.published_date::date +'180 day'::interval as date) >= thlmc.date_of_service::date-- First Fill DOS must be less than or equal to the published date + 180 days 
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
		)
...
select distinct ttp.patient_identifier 
	...
	,ttp.mml_med_super_concept
	,ttp.is_first_fill
	,ttp.published_date||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id as priority_index
...
left join temp_hzn_existing_med_found_keys themdk on alt.med_id = themdk.med_id and themdk.mml_claim_date::date >= thlmc.date_of_service::date --for indentifying Tier 1 and Existing Med Found within same month
left join temp_recurring_multi_dsr_same_claim trmdsc on thlmc.id = trmdsc.mml_record_id
...
from temp_hzn_existing_med_found_keys themdk
...
left join rpt_dsrd_transaction_detail dtd on aaa.transaction_id = dtd.transaction_id
...
select ef.mml_record_id,max(ef.published_date::text||ef.med_id::text) as most_recent_ci
from temp_hzn_existing_med_found_list ef
...
select now() as "ReportDate"
	...
	,ef.published_date
	,ef.mml_record_id
	,tdr.dsr_run_history_id
from temp_dsr_report tdr
join temp_hzn_existing_med_found_list ef on tdr.med_id = ef.med_id
join temp_emf_prep tep on ef.published_date::text||ef.med_id::text = tep.most_recent_ci
left join temp_new_ci_list tncpl on ef.med_id = tncpl.med_id or ef.mml_record_id = tncpl.mml_record_id
left join temp_recurring_list tck on ef.med_id = tck.med_id or ef.mml_record_id = tck.mml_record_id
where tncpl.med_id is null and tck.med_id is null
UNION ALL 
select now() as "ReportDate"
	...
	,case 
		when lower(tdr.requested_existing_med_type) = 'm' and tr.alt_annual_total_savings > 30000 then 30000
		when lower(tdr.requested_existing_med_type) = 'p' and tr.alt_annual_total_savings > 11250 then 11250
		when lower(tdr.requested_existing_med_type) = 'a' and tr.alt_annual_total_savings > 2500 then 2500
		else tr.alt_annual_total_savings
	end as "AdjustedTotalSavings"
	,tdr.line_of_business
	,'Recurring' as fill_type
	,tr.published_date
	,tr.mml_record_id
	,tdr.dsr_run_history_id
from temp_dsr_report tdr
...
--drop table if exists rpt_hzn_dsrd_claim_gpi_outcome_prep_data;
--create table rpt_hzn_dsrd_claim_gpi_outcome_prep_data as
INSERT INTO public.rpt_hzn_dsrd_claim_gpi_outcome_prep_data
	(
		...
		, "Alt3DrugNDC", "Alt3DrugPA", "Alt3DrugSPID", "Alt3DrugConceptName", "Alt3DrugQuantity", "Alt3DrugDaysSupply", "ExistingMedInternalID", requested_existing_gpi, mml_gpi, alt1_gpi
		, alt2_gpi, alt3_gpi, requested_existing_patient_pay_amount, requested_existing_total_cost, mml_patient_cost, mml_total_cost, alt1_patient_pay_amount, alt1_total_cost, alt2_patient_pay_amount
		, alt2_total_cost, alt3_patient_pay_amount, alt3_total_cost, patient_name, member_id, carrier_id, account_id, group_id, requested_existing_super_concept, mml_med_super_concept
		, "AdjustedPatientSavings", "AdjustedTotalSavings", line_of_business, fill_type, published_date, mml_record_id, dsr_run_history_id, "PA_avoided"
	)
select DISTINCT
	t."ReportDate"
...
from temp_output t
	left join rpt_hzn_dsrd_claim_gpi_outcome_prep_data rhdcgop on t.mml_record_id = rhdcgop.mml_record_id
	where rhdcgop."ExistingMedInternalID" is null
;

--create index "_I_rpt_hzn_dsrd_claim_gpi_outcome_prep_data" on rpt_hzn_dsrd_claim_gpi_outcome_prep_data using btree("ExistingMedInternalID","SwitchChangeMonth")
--;
--create index "_I_rpt_hzn_dsrd_claim_gpi_outcome_prep_data_mml_record_id" on rpt_hzn_dsrd_claim_gpi_outcome_prep_data using btree(mml_record_id)
--;
------------End Reporting-------------
COMMIT;$$);
