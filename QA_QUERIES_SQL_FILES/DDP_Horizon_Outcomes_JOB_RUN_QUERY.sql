...
VALUES ('DDP_Horizon_Outcomes_JOB_RUN_QUERY', $$BEGIN;
...
select max(bt.id) as id,ic.bin, ic.pcn, phy.npi, lower(trim(ip.cardholder_id)) as cardholder_id, pat.birthdate::date, pm.ndc, pm.quantity, pharm.ncpdp_id, bt.inserted_at::date
...
select distinct a.id, trim(cm.member_id) as member_id
from temp_ddp_dedup a
join rpt_hzn_claim_history cm on a.birthdate::date = right(cm.new_patient_identifier,10)::date
	and ((lower(a.cardholder_id) = lower(cm.member_id)) or
			(length(a.cardholder_id) > 11 and length(cm.member_id) > 11 and lower(a.cardholder_id) like '%3hzn%' and  lower(SUBSTRING(a.cardholder_id,POSITION('3hzn' IN lower(a.cardholder_id)),12)) = lower(SUBSTRING(cm.member_id,POSITION('3hzn' IN lower(cm.member_id)),12)))
		)
;
...
select distinct 
lower('hzn' || trim(tmc.member_id) || tp.birthdate::date)as patient_identifier --use XXX to avoid wrong matching
,tp.inserted_at::date as report_created_date
...
,pe2.generic_drug_item_name as alt_generic_name
...
left join product_equivalencies pe2 on pl2.specific_product_id = pe2.specific_product_id
left join rpt_hzn_ddp_claim_gpi_outcome_prep_data rhdcgop on pm.id = rhdcgop."ChosenMedInternalID" and rhdcgop.fill_type = 'New'
...
select rhch.*
,substring(rhch.gpi_14,1,12) as gpi_12
,pe.concept_name
,pe.super_concept as med_super_concept
,case 
	when rhch.days_supply > 30 then rhch.days_supply
	else 30
...
where rhch.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval)- '240 day'::interval as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)
;
...
;
create index "_I_temp_hzn_claims_patient_identifier_dos_med_super_concept" on temp_hzn_claims using btree(new_patient_identifier,date_of_service,med_super_concept,gpi_12)
;
----------- End Claim History ---------------------

------------Begin Reporting-------------
------------Begin identifying ddp with chosen medication claim found ------------------------------------------------
drop table if exists temp_hzn_chosen_med_found_list;
create temp table temp_hzn_chosen_med_found_list as
...
join temp_hzn_claims thlmc on alt.patient_identifier = thlmc.new_patient_identifier 
	and (
			...
			or trim(lower(alt.med_display_name)) = trim(lower(thlmc.med_name)) -- Chosen Medication & Resulting Medication with same med name
		)
where thlmc.date_of_service::date >= alt.report_created_date::date --DDP allow same date DOS 
...
drop table if exists temp_hzn_chosen_med_found_keys;
create temp table temp_hzn_chosen_med_found_keys as
select distinct med_id,mml_claim_date
from temp_hzn_chosen_med_found_list
;

create index "_I_temp_hzn_chosen_med_found_keys_med_id" on temp_hzn_chosen_med_found_keys using btree(med_id,mml_claim_date)
;
...
select distinct thlmc.new_patient_identifier as patient_identifier
	...
	,case
		...
		when lower(alt.med_med_type) = 'a' and ((alt.med_total_cost-thlmc.total_cost) >= -250::float8) then (alt.med_total_cost-thlmc.total_cost)
		...
		when lower(alt.med_med_type) = 'a' and ((alt.med_total_cost-thlmc.total_cost) < -250::float8) then -250::float8
...
left join temp_hzn_chosen_med_found_keys themdk on alt.med_id = themdk.med_id and themdk.mml_claim_date::date >= thlmc.date_of_service::date
where cast(alt.report_created_date::date +'120 day'::interval as date) >= thlmc.date_of_service::date-- First Fill DOS must be less than or equal to the DDP inserted date + 120 days 
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
		)
		and themdk.med_id is null -- no Chosen med found after resulting med
;
...
select distinct ttp.patient_identifier 
	...
	,ttp.report_created_date||LPAD(ttp.alt_annual_total_savings::text,10,'0')||ttp.med_id as priority_index
from temp_new_tier1_prematches ttp
left join temp_new_tier1_prematches ttpf on ttp.med_id = ttpf.med_id and ttpf.lookback_days > 0 and ttpf.lookback_days < 120
...
;
create index "_I_temp_t1_multi_ddp_same_claim_mml_record_id" on temp_t1_multi_ddp_same_claim using btree(mml_record_id)
;
create index "_I_temp_t1_multi_ddp_same_claim_max_priority_index" on temp_t1_multi_ddp_same_claim using btree(max_priority_index)
;
...
join temp_t1_multi_ddp_same_claim trmdsc on trm.priority_index = trmdsc.max_priority_index and trm.mml_record_id = trmdsc.mml_record_id
;
...
join temp_hzn_claims thlmc on alt.patient_identifier = thlmc.new_patient_identifier 
	...
	and (alt.med_super_concept is not null
			...
			and upper(alt.med_super_concept) = upper(thlmc.med_super_concept)) --Super Concept match between chosen and Claim Med (resulting med)
left join temp_hzn_claims thc on alt.patient_identifier = thc.new_patient_identifier 
	and thc.date_of_service::date <= alt.report_created_date::date
	and trim(lower(alt.med_display_name)) <> trim(lower(thc.med_name))
	and (thlmc.gpi_12 is not null 
...
left join temp_hzn_chosen_med_found_keys themdk on alt.med_id = themdk.med_id and themdk.mml_claim_date::date >= thlmc.date_of_service::date --for indentifying Tier 1 and chosen Med Found within same month
left join temp_t1_multi_ddp_same_claim tt1mdsc on thlmc.id = tt1mdsc.mml_record_id
where cast(alt.report_created_date::date +'120 day'::interval as date) >= thlmc.date_of_service::date-- First Fill DOS must be less than or equal to the DDP inserted date + 120 days 
		and ((thlmc.date_of_service::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date)) --Only claims for previous month
			or (thlmc.adjudication_datetime::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
			or (thlmc.inserted_at::date between cast(date_trunc('month', current_date - '1 month'::interval) as date) and cast(date_trunc('month', current_date)- '1 second'::interval as date))
...
left join temp_tier2_prematches ttpf on ttp.med_id = ttpf.med_id and ttpf.lookback_days > 0 and ttpf.lookback_days < 120
...
create index "_I_temp_t2_multi_ddp_same_claim_mml_record_id" on temp_t2_multi_ddp_same_claim using btree(mml_record_id)
;
create index "_I_temp_t2_multi_ddp_same_claim_max_priority_index" on temp_t2_multi_ddp_same_claim using btree(max_priority_index)
;
...
join temp_t2_multi_ddp_same_claim trmdsc on trm.priority_index = trmdsc.max_priority_index and trm.mml_record_id = trmdsc.mml_record_id
;
...
from temp_hzn_chosen_med_found_keys themdk
...
select patient_id,line_of_business,insurance_company_id
from public.insurance_policies
;

create index "_I_temp_insurance_policies" on temp_insurance_policies using btree(patient_id)
;
create index "_I_temp_insurance_policies_insurance_company_id" on temp_insurance_policies using btree(insurance_company_id)
;
...
join public.priced_medications pm on bt.id = pm.beacon_transaction_id and pm.chosen_med = true
join public.providers phy on bt.provider_id = phy.id
join temp_insurance_policies ip on pat.id  = ip.patient_id
join public.insurance_companies	ic on ip.insurance_company_id = ic.id
join pbm_logs pls on bt.request_id = pls.request_id and pls.is_final = true 
join temp_ci_keys ett on pm.id = ett.med_id --Matched results chosen_med_found + Tier 1 + Tier 2
...
where lower(pay."name") = 'horizon' 
	...
	and cast(bt.inserted_at::date +'120 day'::interval as date) >= cast(date_trunc('month', current_date - '1 month'::interval) as date)
;
create index "_I_temp_ddp_dedup_report_1" on temp_ddp_dedup_report using btree(med_id)
;
...
select ef.mml_record_id,max(ef.report_created_date::text||ef.med_id::text) as most_recent_ci
from temp_hzn_chosen_med_found_list ef
...
join temp_hzn_chosen_med_found_list ef on tdr.med_id = ef.med_id
join temp_emf_prep tep on ef.report_created_date::text||ef.med_id::text = tep.most_recent_ci
left join temp_new_ci_list tncpl on ef.med_id = tncpl.med_id or ef.mml_record_id = tncpl.mml_record_id
where tncpl.med_id is null
UNION ALL 
select now() as "ReportDate"
	...
	,case 
		when lower(tdr.requested_chosen_med_type) = 'm' and t1.alt_annual_total_savings > 30000 then 30000
		when lower(tdr.requested_chosen_med_type) = 'p' and t1.alt_annual_total_savings > 11250 then 11250
		when lower(tdr.requested_chosen_med_type) = 'a' and t1.alt_annual_total_savings > 2500 then 2500
...
select now() as "ReportDate"
	...
	,case 
		when lower(tdr.requested_chosen_med_type) = 'm' and t2.alt_annual_total_savings * 0.75 > 30000 then 30000
		when lower(tdr.requested_chosen_med_type) = 'p' and t2.alt_annual_total_savings * 0.75 > 11250 then 11250
		when lower(tdr.requested_chosen_med_type) = 'a' and t2.alt_annual_total_savings * 0.75 > 2500 then 2500
	...
	,t2.mml_record_id
from temp_ddp_dedup_report tdr
join temp_new_ci_list t2 on tdr.med_id = t2.med_id and t2.most_savings_order = 1 and t2.change_type = 'Tier 2'
where t2.fil_type_code = 1
;
---------------End Generate Output Report -----------------------------------

--drop table if exists rpt_hzn_ddp_claim_gpi_outcome_prep_data;
--create table rpt_hzn_ddp_claim_gpi_outcome_prep_data as
INSERT INTO public.rpt_hzn_ddp_claim_gpi_outcome_prep_data
	(
		...
		, "ClaimDrugDaysSupply", "ClaimDrugMaintenancePeriodic", "AnnualPatientSavings", "AnnualTotalSavings", "SwitchChangeMonth", uid, "ChangeType", org, "ProductLine", "Alt1Drug", "Alt1DrugGenericName"
		, "Alt1DrugBrand", "Alt1DrugNDC", "Alt1DrugPA", "Alt1DrugSPID", "Alt1DrugConceptName", "Alt1DrugQuantity", "Alt1DrugDaysSupply", "Alt2Drug", "Alt2DrugGenericName", "Alt2DrugBrand", "Alt2DrugNDC"
		, "Alt2DrugPA", "Alt2DrugSPID", "Alt2DrugConceptName", "Alt2DrugQuantity", "Alt2DrugDaysSupply", "Alt3Drug", "Alt3DrugGenericName", "Alt3DrugBrand", "Alt3DrugNDC", "Alt3DrugPA", "Alt3DrugSPID"
		, "Alt3DrugConceptName", "Alt3DrugQuantity", "Alt3DrugDaysSupply", "ChosenMedInternalID", requested_chosen_gpi, mml_gpi, alt1_gpi, alt2_gpi, alt3_gpi, requested_chosen_patient_pay_amount
		, requested_chosen_total_cost, mml_patient_cost, mml_total_cost, alt1_patient_pay_amount, alt1_total_cost, alt2_patient_pay_amount, alt2_total_cost, alt3_patient_pay_amount, alt3_total_cost
		, carrier_id, account_id, group_id, requested_chosen_super_concept, mml_med_super_concept, "AdjustedPatientSavings", "AdjustedTotalSavings", line_of_business, fill_type, mml_record_id, "PA_avoided"
	)
select distinct
		t."ReportDate"
...
from temp_output t
		left join rpt_hzn_ddp_claim_gpi_outcome_prep_data rhdcgop on t.mml_record_id = rhdcgop.mml_record_id
		where rhdcgop."ChosenMedInternalID" is null
;

--create index "_I_rpt_hzn_ddp_claim_gpi_outcome_prep_data" on rpt_hzn_ddp_claim_gpi_outcome_prep_data using btree("ChosenMedInternalID","SwitchChangeMonth")
--;
--create index "_I_rpt_hzn_ddp_claim_gpi_outcome_prep_data_mml_record_id" on rpt_hzn_ddp_claim_gpi_outcome_prep_data using btree(mml_record_id)
--;
...
from public.rpt_hzn_ddp_claim_gpi_outcome_prep_data rbd
...
--167328
UPDATE rpt_hzn_ddp_claim_gpi_outcome_prep_data
SET "ChangeType" = 'NAS' 
FROM temp_remove_BtoC
WHERE temp_remove_BtoC.uid = rpt_hzn_ddp_claim_gpi_outcome_prep_data.uid
	and rpt_hzn_ddp_claim_gpi_outcome_prep_data."ChangeType" in ('Tier 1','Tier 2');
------------End Reporting-------------
COMMIT;$$);
